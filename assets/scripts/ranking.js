...
(function () {
    ...
    return new Promise(function (resolver, rechazar) {
      ...
      function limpiar() {
        ...
        try {
          ...
          window[nombreCb] = undefined;
        }
        if (script.parentNode) script.parentNode.removeChild(script);
      }

      window[nombreCb] = function (datos) {
        limpiar();
        resolver(datos);
      };
      script.onerror = function () {
        limpiar();
        rechazar(new Error("sin_respuesta"));
      };
      script.src =
        base + "?juego=" + encodeURIComponent(juego) + "&callback=" + nombreCb;
  ...
  function tabla(juego, top) {
    ...
    var filas = top
      .map(function (f) {
        var celdas = esReaccion
          ? '<td class="dato">' +
            f.puntos +
            '</td><td class="dato oculta-chico">' +
            (f.reaccion ? f.reaccion + " s" : "–") +
            "</td>"
          : '<td class="dato">' +
            (f.efecto > 0 ? "+" : "") +
            f.efecto +
            ' ms</td><td class="dato oculta-chico">' +
            f.precision +
            "%</td>";
        return (
          '<tr class="' +
    ...
    return (
      '<table class="rank-tabla"><thead><tr><th></th><th>Jugador</th>' +
      encabezado +
      "</tr></thead><tbody>" +
      filas +
      "</tbody></table>" +
    ...
    pedirTop(juego)
      .then(function (datos) {
        if (!datos || datos.ok === false)
          return (caja.innerHTML = aviso("No se pudo leer la tabla."));
        if (!datos.top || !datos.top.length) {
          return (caja.innerHTML = aviso(
            "Todavía no hay resultados cargados para este módulo. El primero que lo juegue abre la tabla.",
          ));
        }
